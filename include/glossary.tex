...
\newglossaryentry{difference} {
  ...
  description={所有属于 $A$ 但不属于~$B$ 的{元素}的集合：$A \setminus B = \Setabs{x}{x \in A \text{ and }
...
\newglossaryentry{range}{
  ...
  description = {陪域中实际被 $f$ 输出的那些元素构成的子集；$\ran{f} = \Setabs{y \in B}{f(x) = y \text{ for some $x \in
...
\newglossaryentry{enumeration} {
  name=枚举,  
  description={一个集合~$A$ 的所有\gls{element}的可能无限的列表；形式化地说，是一个满射函数
...
\newglossaryentry{sentence} {
  name=语句,
  description={不含\gls{free}变元的{formula}（参见
    \olref[fol][syn][fvs]{sec}）} }

\newglossaryentry{variable assignment} {
  name=变元指派,
  description={将每个{变量}映射到~$\Domain M$ 中一个元素的 \gls{function}（参见 \olref[fol][syn][sat]{sec}）} }
...
\newglossaryentry{cs} {
    name=覆盖结构,
    description={一个 \gls{structure}，其论域中每个元素都是某个闭{term}的{value}（参见
...
\newglossaryentry{church-turing thesis} {
  name=Church-Turing（邱奇-图灵）论题,
  description={断言：凡是通过有效程序可计算的，都是 Turing（图灵）可计算的（参见 \olref[tur][mac][ctt]{sec}）} }

\newglossaryentry{completeness} {
  name=完备性,
  description={推导系统的性质；若每当
    $\Gamma$ 语义上后承 $!A$ 时，都存在一个{derivation}可建立 $\Gamma \Proves !A$，则该系统是完备的；等价地说，当且仅当
...
\newglossaryentry{sdomain} {
  ...
  description = {非空集合，\gls{structure} 从中获取指派和{变量}的{value}（参见
    \olref[fol][syn][str]{sec}）} }

\newglossaryentry{halting problem} {
  name=停机问题,
  description={判定（对任意 $e$，$n$） Turing（图灵）机~$M_e$ 在输入~$n$ 个划痕时是否停机的问题（参见
...
\newglossaryentry{ls}{
  name = {Löwenheim-Skolem（勒文海姆-斯科伦）定理},
  description = {断言：每个可满足的语句集都有一个可数模型（参见 \olref[fol][com][dls]{sec}）}}

\newglossaryentry{Church-Turing Theorem}{
  name = {Church-Turing（邱奇-图灵）定理},
  description = {断言：不存在能判定任意给定一阶逻辑语句是否\gls{valid}的 Turing（图灵）机（参见
